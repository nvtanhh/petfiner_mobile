...
class InboxChat extends StatefulWidget {
  ...
  InboxChat(this.group, this.title, {this.user});
  static Future navigate(FbInboxGroupModel group, String title) {
    // navigatorKey in MaterialApp
    return navigatorKey.currentState
        .push(MaterialPageRoute(builder: (context) => InboxChat(group, title)));
  }
...
class _InboxChatState extends State<InboxChat> {
  ...
  void didChangeDependencies() async {
    if (_inboxBloc == null || user == null) {
      ...
      // await getUserFromToken(); // this just to get userId, avatar, name. you can replace this with your params
      user = widget.user;
  ...
  void onIncomingMessage(event) async {
    ...
    fbMessages.removeWhere((message) => message.uid == user.id.toString());
    if (fbMessages.isEmpty) return;
  ...
  void onSend(ChatMessage message) {
    ...
    _updateGroupPageText(widget.group.id, user.name, text, message.createdAt,
        message.user.avatar);
    if (text.length > 0) {
      if (_file == null) {
        _inboxBloc.addMessage(widget.group.id, text, message.createdAt,
            user.id.toString(), user.name, user.avatar);
      } else {
        FileUtil.uploadFireStorage(_file,
                path: 'chats/group_${widget.group.id}/user_${user.id}')
            .then((fileUrl) {
          setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: DashChat(
        ...
        user:
            _users.firstWhere((chatUser) => chatUser.uid == user.id.toString()),
        ...
        onClickMenu: (val) {
          if (val.menuTitle == 'Voice call') {
            Navigator.push(
                ...
                new MaterialPageRoute(
                  builder: (context) =>
                      VoiceCallPage(groupId: widget.group.id, users: _fbUsers),
                ));
          ...
          if (val.menuTitle == 'Video call') {
            ...
            Navigator.push(
                ...
                new MaterialPageRoute(
                  builder: (context) =>
                      VideoCallPage(groupId: widget.group.id, users: _fbUsers),
                ));
          }
        },
        stateChanged: (val) {},
